...
class WHCategoryFields extends Module
{
    ...
    public function hookDisplayHeaderCategory($params)
    {
        ...
        if(!is_null($extension)){
            $this->context->smarty->assign([
                'short_text' => $extension->getExtensionLangByLangId($idLang)
                    ->getShortText(),
                'long_text' => $extension->getExtensionLangByLangId($idLang)
                    ->getLongText(),
            ]);
            if($extension->getFilename()!=""){
                $this->context->smarty->assign('filename_path',
                    _MODULE_DIR_."whcategoryfields/views/img/uploads/".$extension->getFilename());
            }
        ...
        if(isset($params['id']) && $params['id']>0){
            ...
            if(!is_null($extension)){
                ...
                if(isset($extension) && $extension->getFileName()!=""){
                    $link = New Link();
                    $deletionLink = $link->getAdminLink(
                        'AdminWHCategoryFields', true, ['route'=>
                        'admin_whcategoryfields_file_delete', 'idCategory'
                    =>$extension->getIdCategory()], []);
                    $formBuilder->add('image_file_icon', CustomContentType::class, [
                        ...
                        'data' => [
                            ...
                            'filenameUrl' => $link->getBaseLink()."/modules/"
                                .$this->name."/views/img/uploads/"
    ...
    {
        return Db::getInstance()->execute('CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ .
            'whcategoryfields_extension_lang` (
    ...
    private function updateWHFieldsExtension(array $params)
    {
        ...
        if(!is_null($fileToUpload)){
            ...
            try {
                $fileToUpload->move(
                    _PS_MODULE_DIR_."whcategoryfields/views/img/uploads/",$filename);
        ...
        if(is_null($extension)){
            $this->commandBus->handle(new AddExtensionCommand($categoryId,$shortText,$longText,$filename));
        }else{
            $this->commandBus->handle(new EditExtensionCommand($extension->getId(),$categoryId,$shortText,$longText,$filename));
        }
    }
}